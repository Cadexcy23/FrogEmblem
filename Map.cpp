...
{
	//set to exhausted
	Map::activeMap.entityList.characters[Map::activeMap.selectedCharacterID].exhausted = true; //maybe make a func for this so we can tex color mod
	//deselect unit
...
void loadCharacterMenuButtons()
{
	...
	//load each button
	std::vector<SDL_Texture*> attackTex = { Artist::loadTexture("Resource/menus/character/menuAttack0.png"), Artist::loadTexture("Resource/menus/character/menuAttack1.png") };
	Controller::buttonList.push_back(Button::makeButton(attackTex, start.x, start.y, 100, dummyFunc));

	std::vector<SDL_Texture*> itemsTex = { Artist::loadTexture("Resource/menus/character/menuItems0.png"), Artist::loadTexture("Resource/menus/character/menuItems1.png") };
	Controller::buttonList.push_back(Button::makeButton(itemsTex, start.x, start.y + 64 * 1, 100, dummyFunc));

	std::vector<SDL_Texture*> waitTex = { Artist::loadTexture("Resource/menus/character/menuWait0.png"), Artist::loadTexture("Resource/menus/character/menuWait1.png") };
	Controller::buttonList.push_back(Button::makeButton(waitTex, start.x, start.y + 64 * 2, 100, wait));
...
void Map::draw()
{
	if (Controller::state == Controller::STATE_MAP)
	{
		...
		//draw paths for selected char
		if (Map::activeMap.selectedCharacterID >= 0)
		{
			Character::character* charPoint = &Map::activeMap.entityList.characters[Map::activeMap.selectedCharacterID];
			Artist::drawImage(charPoint->pathGraphic, (charPoint->x - charPoint->moveDist - 2) * 256 - Map::activeMap.camOffset.x, (charPoint->y - charPoint->moveDist - 2) * 256 - Map::activeMap.camOffset.y);
		}
		...
		{
			drawTileInfo({ 64, 64 }, { Map::activeMap.entityList.characters[Map::activeMap.selectedCharacterID].x * 256 - Map::activeMap.camOffset.x, Map::activeMap.entityList.characters[Map::activeMap.selectedCharacterID].y * 256 - Map::activeMap.camOffset.y }, SDL_FLIP_HORIZONTAL);
		}
		...
		else if (activeMap.menuState == MENU_CHARACTER)
		{
			Artist::drawImage(activeMap.mapCharacterHead, 1536, 384 - 64);
			Artist::drawImage(activeMap.mapCharacterFoot, 1536, 384 + 64 * 3);
			Button::updateButtonVector(Controller::buttonList);
		}
...
void Map::controller()//MOVE SOME FUNCTIUONALITY TO THIER OWN FUNCTIONS/IN THIER STRUCT
{
	if (Controller::state == Controller::STATE_MAP)
	{
		...
		{
			if (activeMap.menuState == MENU_NONE)//MAKE SWITCH CASE OF STATES
			{
				...
				if (Map::activeMap.selectedCharacterID >= 0 && Map::activeMap.selectedCharacterID < Map::activeMap.entityList.characters.size())
				{
					...
					//check if we click on ourself
					if (mousePoint.x == tempChar->x && mousePoint.y == tempChar->y)
					{
						//OPEN CHARACTER MENU
						Map::activeMap.menuState = Map::MENU_CHARACTER;
						loadCharacterMenuButtons();
						taken = true;
					}
					for (int i = 0; i < activeMap.entityList.characters.size(); i++)
					{
						if (mousePoint.x == activeMap.entityList.characters[i].x && mousePoint.y == activeMap.entityList.characters[i].y)//if we click on another unit
						{
							//error noise?
							taken = true;
						}
					...
					if (!taken)
					{
						//MOVE CHAR and set menu state
						tempChar->x = mousePoint.x;
						tempChar->y = mousePoint.y;
						loadCharacterMenuButtons();
						activeMap.menuState = MENU_CHARACTER;
						//TEMP
						tempChar->genPaths();
					}
				}
				else
				{
					...
					for (int i = 0; i < Map::activeMap.entityList.characters.size(); i++)
					{
						if (Map::activeMap.entityList.characters[i].x == mousePoint.x && Map::activeMap.entityList.characters[i].y == mousePoint.y)
						{
							...
							activeMap.charStartPoint = { activeMap.entityList.characters[i].x , activeMap.entityList.characters[i].y };
						}
					}
		...
		if (Controller::keyboardStates[SDL_SCANCODE_ESCAPE] == 1)
		{
			...
			case MENU_CHARACTER:
				//undo movement
				activeMap.entityList.characters[activeMap.selectedCharacterID].x = activeMap.charStartPoint.x;
				activeMap.entityList.characters[activeMap.selectedCharacterID].y = activeMap.charStartPoint.y;
				//regen paths
				Map::activeMap.entityList.characters[Map::activeMap.selectedCharacterID].genPaths();
